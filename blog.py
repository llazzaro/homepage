...
class BlogHandler(webapp.RequestHandler):
  ...
  def get(self):
    entries = db.Query(Entry).filter('draft =',False).order('-published_at').fetch(limit=10)
    admin = users.is_current_user_admin()
    return self.response,{'admin':admin,'entries':entries,'nombre_seccion':'Blog'}
...
class TagHandler(webapp.RequestHandler):
  @templateSelector('templates/blog/blog.html')
  def get(self,tag):
    entries = db.Query(Entry).filter('tags =',tag).order('-published_at').fetch(FETCH_THEM_ALL)
    return self.response,{'entries':entries}
...
def main():
  application = webapp.WSGIApplication([
		...
		(r'/blog/new',EntryHandler),
		(r'/blog/edit/?',EntryHandler),
		(r'/blog/delete/?',DeleteEntryHandler),
